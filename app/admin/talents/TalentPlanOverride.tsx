...
import { updateTalentSubscriptionPlan } from "@/app/admin/actions";
...
export default function TalentPlanOverride({
  profileId,
  talentName,
  currentPlan
}: {
  profileId: string;
  talentName: string;
  currentPlan: TalentSubscriptionPlan;
}) {
  ...
  function handlePlanChange(nextPlan: TalentSubscriptionPlan) {
    ...
    startTransition(() => {
      void updateTalentSubscriptionPlan(formData)
        .then((result) => {
          if (!result.ok) {
            setSelectedPlan(currentPlan);
            setToast({
          ...
          setToast({
            type: "success",
            message: `${talentName} 已調整為 ${nextPlan} 方案。`
          });
          router.refresh();
        })
        .catch((error: unknown) => {
  ...
  return (
    <div className="relative">
      ...
      <label className="sr-only" htmlFor={`talent-subscription-plan-${profileId}`}>
        調整 {talentName} 的訂閱方案
      </label>
      <div className="flex items-center gap-2">
        <select
          id={`talent-subscription-plan-${profileId}`}
          value={selectedPlan}
          onChange={(event) =>
            handlePlanChange(event.target.value as TalentSubscriptionPlan)
          }
          disabled={isPending}
