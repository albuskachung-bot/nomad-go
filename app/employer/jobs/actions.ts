...
export async function createEmployerJob(formData: FormData) {
  let redirectUrl = "/employer/jobs?created=1";
  ...
  try {
    ...
    redirectUrl = `/employer/jobs/create?error=${encodeError(
...
export async function updateEmployerJob(jobId: string, formData: FormData) {
  let redirectUrl = "/employer/jobs?updated=1";
  ...
  try {
    ...
    redirectUrl = `/employer/jobs/${jobId}/edit?error=${encodeError(
      getErrorMessage(error)
    )}`;
  }
...
export async function toggleJobStatus(jobId: string, newStatus: JobStatus) {
  if (!validJobStatuses.has(newStatus)) {
    redirect(
      `/employer/jobs?error=${encodeError("不支援的職缺狀態。")}`
    );
  }

  let redirectUrl = "/employer/jobs?status-updated=1";
  ...
  try {
    ...
    redirectUrl = `/employer/jobs?error=${encodeError(
      getErrorMessage(error)
    )}`;
  }
...
export async function deleteJob(jobId: string) {
  let redirectUrl = "/employer/jobs?deleted=1";
